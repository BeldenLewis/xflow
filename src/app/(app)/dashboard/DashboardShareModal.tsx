...
export function DashboardShareModal({ open, onClose, projectId, projectName }: Props) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/projects/${projectId}/dashboard-share`);
      if (!res.ok) {
            ...
            <div className="mb-4 flex items-start justify-between">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-base font-semibold">실시간 대시보드 공유</h2>
                </div>
                <p className="mt-1 text-xs text-muted-foreground">{projectName} · 최근 30일 실시간 등록 현황을 읽기 전용으로 공유합니다</p>
              </div>
              <button
                onClick={onClose}
